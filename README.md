...
**2015년 이후 상장폐지된 종목도 포함합니다** — 아래 「상장폐지 종목」 참조.
...
| `DelistingDate` | string | 상장폐지일 (`YYYY-MM-DD`). **상장 중인 종목은 null** |
| `DelistingReason` | string | 상장폐지 사유. 상장 중인 종목은 null |

### 상장폐지 종목

마스터는 Seibro 엑셀(**현재 상장 종목**)에서 만들어지므로, 그대로 두면 폐지된 종목은
조회 대상 목록에조차 오르지 않습니다 — 릴리즈에서 빠진 게 아니라 **처음부터 수집되지
않습니다.** 그 결과가 생존편향입니다.

이게 결측 행보다 더 넓게 번지는 이유는 **`MRS_*` 가 일자별 횡단면 백분위**이기
때문입니다. 살아남은 종목만으로 순위를 매기면 **살아남은 종목의 백분위까지 틀립니다** —
어느 해 상위 30% 였던 종목이, 그 해 사라진 종목들을 뺀 모집단에서는 상위 20% 로 보입니다.
소비자가 나중에 폐지 종목 시세를 덧붙여도 **백분위는 고쳐지지 않습니다.**

그래서 `build_krx_stock_master.py` 가 `fdr.StockListing('KRX-DELISTING')`(**벌크 1회
조회**)로 폐지 종목을 마스터에 넣습니다.

| | |
|---|---|
| 기본 범위 | 2015-01-01 이후 폐지 · `SecuGroup == '주권'` · KOSPI/KOSDAQ |
| 옵션 | `--no-delisted` · `--delisted-from` · `--include-konex` |
| 업종 | **null** — 폐지 목록의 업종은 단일 레벨이라 Seibro 의 대/중/소 분류와 맞지 않습니다. ETF 와 같은 처리이고 `industry.py` 가 `"Unknown"` 으로 다룹니다 |

⚠️ **Seibro 엑셀은 시간이 지나면 낡습니다.** 작성 시점에 KRX 가 이미 폐지한 2026년 종목
61개가 엑셀에는 아직 상장 중으로 남아 있었습니다. 그래서 단순 concat 이 아니라 **병합**
합니다 — 코드가 양쪽에 있으면 엑셀 행(업종·주식수가 풍부)을 유지하되 **폐지일과 사유를
찍습니다.** 빌드 로그가 그 건수를 경고로 출력합니다.
